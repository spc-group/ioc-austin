...
class RobotAxisFields(MotorFieldsBase):
    pass

class RobotJointFields(MotorFieldsBase):
    pass


autosave_fields = ["user_high_limit", "user_low_limit", "tweak_step_size", "description",]
...
class RobotJoint(PvpropertyDouble):
    def __init__(self, *args, axis_num: int = 99, **kwargs):
        self.axis_num = axis_num
        super().__init__(*args, **kwargs)
    ...
    async def read_motor(self):
        """Read the current motor position from the urx driver."""
        # Get current join positions
        new_joints = await self.loop.run_in_executor(None, self.group.parent.driver.get_joints)
        # Get just the PV for this one axis
        return new_joints[self.axis_num]

    async def do_move(self, target, speed):
        """Callback to move to a given set of joint coordinates.

        Reads target joint position from other PVs:

        """
        # Build new coordinate position
        new_joints = []
        for axis in ["i", "j", "k", "l", "m", "n"]:
            jnt_val = getattr(self.group, f"{axis}").fields["RBV"].value
            new_joints.append(jnt_val)
        new_joints[self.axis_num] = target
        # Get motion parameters from parent
        acceleration = self.group.acceleration.value
        velocity = self.group.velocity.value
        # Move to the new joint position
        do_mov = partial(
            self.group.parent.driver.movej,
            joints=new_joints,
            acc=acceleration,
            vel=velocity,
        )
        await self.loop.run_in_executor(None, do_mov)
        return target
    


class RobotAxis(RobotJoint):
        
    async def read_motor(self):
        """Read the current motor position from the urx driver."""
        loop = asyncio.get_running_loop()
        # Get current join positions
        new_positions = await loop.run_in_executor(None, self.group.parent.driver.get_position)
        # Get just the PV for this one axis
        return new_positions[self.axis_num]
    ...
    async def do_move(self, target, speed):
        ...
        # Move to the new joint position
        do_mov = partial(
            self.group.parent.driver.movel,
            pos=new_joints,
            acc=acceleration,
            vel=velocity,
        )
